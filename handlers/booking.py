...
async def clear_history_button(message: Message):
    ...
    # Приветствие с восстановлением командных кнопок (reply-меню)
    await message.answer(
        f"{display_name}, рад вас видеть вновь в нашей системе бронирования переговорных комнат!\n\n"
        "Если хотите использовать функции бота — выберите действие внизу, нажав на кнопку в меню.",
        reply_markup=main_menu_keyboard(),
    )

    # Отдельным сообщением сразу показываем выбор комнаты (inline-кнопки)
    await message.answer(
        "Выберите переговорную комнату:",
        reply_markup=rooms_keyboard(rooms),
    )
